...
//	if (card->ext_csd.rev > 5) { //for mmc to EMMC
#if 0
...
#else
	printk("%s: CSD version %d\n", mmc_hostname(card->host), card->ext_csd.rev); //for mmc to Emmc
#endif
...
int mmc_attach_mmc(struct mmc_host *host, u32 ocr)
{
	...
	if (mmc_host_is_spi(host)) {
		...
		if (err){
            printk("%s: ---- mmc_spi_read_ocr failed \n", __func__);   //add for boot failed
			goto err;
		}
	}
	 ...
	 */
	if (ocr & 0x7F) {
		printk(KERN_WARNING "%s:%s: card claims to support voltages "
	...
	if (err){
	    printk("%s: ---- mmc_init_card failed \n", __func__);   //add for boot failed
		goto err;
	}
	mmc_release_host(host);
	err = mmc_add_card(host->card);
	if (err){
	    printk("%s: ---- mmc_add_card failed \n", __func__);   //add for boot failed
		goto remove_card;
	}
