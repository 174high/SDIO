...
#include <mach/pinctrl.h>	/*for ti wifi*/
#include <linux/mx28_flc_pins.h>	/*for ti wifi*/
...
mxs_set_sclk_speed(struct mxs_mmc_host *host, unsigned int hz)
{
	...
	if (mmc_data && mmc_data->setclock) {
		...
		dev_dbg(host->dev, "Setting clock rate to %d Hz", host->clkrt);
...
void wl12xx_set_power_mmc_priv(bool power_on)
{
...
/***add for ti wifi**/
	switch (ios->power_mode) {
		case MMC_POWER_ON:
		case MMC_POWER_UP:
			if (mmc_data->set_power && (mmc_data->caps & (1<<14)))
				wl12xx_set_power_mmc_priv(true);

		break;

		case MMC_POWER_OFF:
			if (mmc_data->set_power && (mmc_data->caps & (1<<14)))
				wl12xx_set_power_mmc_priv(false);
...
/*patch 130116 add by Gavin*/
#if 0
        if (ios->bus_width & MMC_BUS_WIDTH_DDR) {
               __raw_writel(0x00000001, host->ssp_base + HW_SSP_DDR_CTRL);
               __raw_writel(0x00000031, host->ssp_base + HW_SSP_DLL_CTRL);  //SLV_DLY_TARGET = 6

               printk(KERN_NOTICE  "HW_SSP_DDR_CTRL=0x%x\n", __raw_readl(host->ssp_base + HW_SSP_DDR_CTRL));
               printk(KERN_NOTICE  "HW_SSP_DLL_CTRL=0x%x\n", __raw_readl(host->ssp_base + HW_SSP_DLL_CTRL));
       }
#endif
/***** add end *****/
}
...
static int __init mxs_mmc_probe(struct platform_device *pdev)
{
	...
	if(mmc_data->caps & MMC_CAP_POWER_OFF_CARD)	//for ti wifi
